...
class pos_config(osv.osv):
    ...
    _columns = {
        ...
        'barcode_product': fields.char('Product Barcodes', size=64,
                                       help='The pattern that identifies product barcodes'),
        'barcode_cashier': fields.char('Cashier Barcodes', size=64,
                                       help='The pattern that identifies cashier login barcodes'),
        'barcode_customer': fields.char('Customer Barcodes', size=64,
                                        help='The pattern that identifies customer\'s client card barcodes'),
        'barcode_price': fields.char('Price Barcodes', size=64,
                                     help='The pattern that identifies a product with a barcode encoded price'),
        'barcode_weight': fields.char('Weight Barcodes', size=64,
                                      help='The pattern that identifies a product with a barcode encoded weight'),
        'barcode_discount': fields.char('Discount Barcodes', size=64,
                                        help='The pattern that identifies a product with a barcode encoded discount'),
    }
    ...
    _constraints = [
        ...
        (_check_company_location, "The company of the stock location is different than the one of point of sale",
         ['company_id', 'stock_location_id']),
        (_check_company_journal, "The company of the sale journal is different than the one of point of sale",
         ['company_id', 'journal_id']),
        (_check_company_payment, "The company of a payment method is different than the one of point of sale",
...
class pos_session(osv.osv):
    ...
    def create(self, cr, uid, values, context=None):
        ...
        currentUser = obj.browse(cr, uid, uid)
        if not currentUser.pos_config:
                raise osv.except_osv(_(u'错误!'),
                         _(
                             u"当前用户没有配置默认的POS终端，不许登录"))
        if not currentUser.pos_config.id == pos_config.id:
                raise osv.except_osv(_(u'错误!'),
                         _(
                             u"当前用户不是该默认POS端的操作员，不许登录"))
...
class pos_order(osv.osv):
            ...
            for posorder in self.browse(cr, uid, ids, context=context):
                if posorder.invoice_id:
                    raise osv.except_osv(_('Error!'), _(
                        "You cannot change the partner of a POS order for which an invoice has already been issued."))
                if vals['partner_id']:
    ...
    def unlink(self, cr, uid, ids, context=None):
        for rec in self.browse(cr, uid, ids, context=context):
            if rec.state not in ('draft', 'cancel'):
                raise osv.except_osv(_('Unable to Delete!'),
    ...
    _columns = {
        ...
        'amount_tax': fields.function(_amount_all, string='Taxes', digits_compute=dp.get_precision('Account'),
                                      multi='all'),
        'amount_total': fields.function(_amount_all, string='Total', digits_compute=dp.get_precision('Account'),
                                        multi='all'),
        'amount_paid': fields.function(_amount_all, string='Paid', states={'draft': [('readonly', False)]},
                                       readonly=True, digits_compute=dp.get_precision('Account'), multi='all'),
        'amount_return': fields.function(_amount_all, 'Returned', digits_compute=dp.get_precision('Account'),
                                         multi='all'),
        'lines': fields.one2many('pos.order.line', 'order_id', 'Order Lines', states={'draft': [('readonly', False)]},
    ...
    def _create_account_move(self, cr, uid, dt, ref, journal_id, company_id, context=None):
        ...
        return self.pool['account.move'].create(cr, uid,
                                                {'ref': ref, 'journal_id': journal_id, 'period_id': period_id[0]},
                                                context=context)
            ...
            for line in order.lines:
                ...
                for t in line.product_id.taxes_id:
                    if t.company_id.id == current_company.id:
                        taxes.append(t)
                computed_taxes = \
                account_tax_obj.compute_all(cr, uid, taxes, line.price_unit * (100.0 - line.discount) / 100.0,
                                            line.qty)['taxes']
                ...
                for tax in computed_taxes:
                    ...
                    if tax_amount < 0:
                        group_key = (
                        tax['ref_tax_code_id'], tax['base_code_id'], tax['account_collected_id'], tax['id'])
                    else:
                        group_key = (tax['tax_code_id'], tax['base_code_id'], tax['account_collected_id'], tax['id'])

                    group_tax.setdefault(group_key, 0)
                    group_tax[group_key] += cur_obj.round(cr, uid, cur, tax['amount']) if round_per_line else tax[
                        'amount']
...
class product_product(osv.osv):
    ...
    def _qty_available_in_charge(self, cr, uid, ids, name, arg, context=None):
        print ids
        res = {}
        location_id = self.pool.get('res.users').browse(cr,uid,uid).pos_config.stock_location_id.id
        for id in ids:
            quants = self.pool.get('stock.quant').read_group(cr, uid, [('product_id', '=', id),('location_id','=',location_id)], ['product_id', 'qty'],
    ...
    _columns = {
        'qty_available_in_charge': fields.function(_qty_available_in_charge, string="qty_available_in_charge", type='float')
    }
